...
def recommend_books():
    ...
    try:
        ...
        if book_title:
            # Find book by title
            idx = books_df[books_df['Judul (Title)'].str.lower() == book_title.lower()].index
            
            if len(idx) == 0:
                # Try partial match
                idx = books_df[books_df['Judul (Title)'].str.lower().str.contains(book_title.lower(), na=False)].index
            ...
            if len(idx) > 0:
                ...
                if genre_preference:
                    genre_books = books_df[books_df['Genre'].str.contains(genre_preference, case=False, na=False)]
                else:
                    genre_books = books_df
                
                recommendations = genre_books.nlargest(num_recommendations, 'Rating (dari 5)').to_dict('records')
            ...
            recommendations = books_df.nlargest(num_recommendations, 'Rating (dari 5)').to_dict('records')
